...
# parser
parser = argparse.ArgumentParser(description="SVAE SMILES generation with BO.")
parser.add_argument(
...
def main(parser_namespace):
    ...
    svae_weights_filepath = os.path.join(svae_path, "weights", "best_rec.pt")
    ...
    for protein_identifier, protein_series in protein_df.iterrows():
        ...
        def evaluate_point(latent_point):
            ...
            smile_set = set()
            logger.info(
                f"generate {number_of_points_per_round} molecules from optimal point"
            )
            t = time()
            ...
            with open(output_filepath, "w") as f:
                ...
                for generated_point_index in range(len(smile_set)):
                    f.write(
